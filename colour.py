...
pl.scatter(10**((23.9-ch2_1700[cc_index_1700])/2.5) *fudgefactor,10**((23.9-ch4_1700[cc_index_1700])/2.5) / (10**((23.9-ch2_1700[cc_index_1700])/2.5)),s=250,marker='*',facecolors='k',edgecolors='k')
pl.scatter(10**((23.9-ch2_ALESS[z_index_ALESS])/2.5),10**((23.9-ch4_ALESS[z_index_ALESS])/2.5) / (10**((23.9-ch2_ALESS[z_index_ALESS])/2.5)),s=10,marker='o',facecolors='k',edgecolors='k')

# pl.scatter(10**(-0.4*(ch2_ALESS-ch1_ALESS)),10**(-0.4*(ch4_ALESS-ch2_ALESS)),s=20,facecolors='m',edgecolors='m', label='ALESS SMGs')
pl.scatter(10**((23.9-ch2_ALESS[z_index_ALESS][z_15])/2.5),10**((23.9-ch4_ALESS[z_index_ALESS][z_15])/2.5) / (10**((23.9-ch2_ALESS[z_index_ALESS][z_15])/2.5)),s=20,facecolors='b',edgecolors='b', label=r'$z<1.5$')
pl.scatter(10**((23.9-ch2_ALESS[z_index_ALESS][z_1525])/2.5),10**((23.9-ch4_ALESS[z_index_ALESS][z_1525])/2.5) / (10**((23.9-ch2_ALESS[z_index_ALESS][z_1525])/2.5)),s=20,facecolors='g',edgecolors='g', label=r'$1.5<z<2.5$')
pl.scatter(10**((23.9-ch2_ALESS[z_index_ALESS][z_25])/2.5),10**((23.9-ch4_ALESS[z_index_ALESS][z_25])/2.5) / (10**((23.9-ch2_ALESS[z_index_ALESS][z_25])/2.5)),s=20,facecolors='r',edgecolors='r', label=r'$z>2.5$')
...
pl.scatter(10**((23.9-ch2_1700[z_1700_noz_IRAC])/2.5)*fudgefactor,10**((23.9-ch4_1700[z_1700_noz_IRAC])/2.5) / (10**((23.9-ch2_1700[z_1700_noz_IRAC])/2.5)),s=250, marker='*',edgecolors='k',facecolors='w',zorder=10)
pl.scatter(10**((23.9-ch2_1700[z_1700_cluster_IRAC])/2.5)*fudgefactor,10**((23.9-ch4_1700[z_1700_cluster_IRAC])/2.5) / (10**((23.9-ch2_1700[z_1700_cluster_IRAC])/2.5)),s=250, marker='*',facecolors='g',edgecolors='k',zorder=10)
pl.scatter(10**((23.9-ch2_1700[z_1700_notcluster_IRAC])/2.5)[0]*fudgefactor,10**((23.9-ch4_1700[z_1700_notcluster_IRAC])/2.5)[0] / (10**((23.9-ch2_1700[z_1700_notcluster_IRAC])/2.5))[0],s=250, marker='*', facecolors='blue',edgecolors='k',zorder=10)
pl.scatter(10**((23.9-ch2_1700[z_1700_notcluster_IRAC])/2.5)[1]*fudgefactor,10**((23.9-ch4_1700[z_1700_notcluster_IRAC])/2.5)[1] / (10**((23.9-ch2_1700[z_1700_notcluster_IRAC])/2.5))[1],s=250, marker='*', facecolors='red',edgecolors='k',zorder=10)
...
#    pl.scatter(x,y,s=10,marker='.',c=zspace,cmap=pl.cm.get_cmap(cmap),vmin=vmin,vmax=vmax,zorder=10, label=label[i])
    pl.plot(x,y,c=col[i],ls='-',label=label[i])
    pl.plot(x[np.where((zspace<1.5))[0]],y[np.where((zspace<1.5))[0]],c='b',ls='-')
    pl.plot(x[np.where((zspace>1.5) & (zspace<2.5))[0]],y[np.where((zspace>1.5) & (zspace<2.5))[0]],c='g',ls='-')
    pl.plot(x[np.where((zspace>2.5))[0]],y[np.where((zspace>2.5))[0]],c='r',ls='-')
...
pl.axhline(y=0.85,linestyle='-',color='orange')
pl.axhline(y=1.53,linestyle='-',color='orange')

pl.ylabel(r'$S_{8.0}/S_{4.5}$')
pl.xlabel(r'$S_{4.5}$ ($\mu$Jy)')
...
#pl.xticks([0.6,1,2])
pl.yticks([0.5,1,2,3,4])
ax.set_yticklabels(['0.5','1','2','3','4'])
#ax.set_xticklabels(['0.6','1','2'])
#pl.rcParams['axes.formatter.min_exponent'] = 100

pl.savefig('../../Figures/Colour/CM_1700.pdf', bbox_inches='tight')
#pl.show()
pl.close()
...
for i in range(len(K)):
    if K[i] > 0 and ch1[i] > 0 and ch2[i] > 0:
        pl.scatter(K[i]-ch1[i],ch1[i]-ch2[i],s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(K[i]-ch1[i],ch1[i]-ch2[i],ID[i][6:],size=12)
#        if ch1[i]-ch2[i]>0.25:
#            print ID[i]
    else:
        pass
pl.savefig('../Figures/Colour/h1700/cc_Kch1_ch1ch2.pdf',bbox_inches='tight')
#pl.show()
pl.close()
'''
...
for i in range(len(K)):
    if ch1[i] > 0 and ch2[i] > 0 and ch3[i] > 0 and ch4[i]>0:
        pl.scatter(ch1[i]-ch2[i],ch3[i]-ch4[i],s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(ch1[i]-ch2[i],ch3[i]-ch4[i],ID[i][6:],size=12)
    #        if ch3[i]-ch4[i]>0.25:
    #            print ID[i]
    else:
        pass
...
for z in zspace:
    flux_ch1 = S[np.where(wavelength*(1+z)>=3.6)[0][0]]
    flux_ch2 = S[np.where(wavelength*(1+z)>=4.5)[0][0]]
    flux_ch3 = S[np.where(wavelength*(1+z)>=5.8)[0][0]]
    flux_ch4 = S[np.where(wavelength*(1+z)>=8.0)[0][0]]

    ch3_ch4.append(-2.5*np.log10(flux_ch3/flux_ch4))
    ch1_ch2.append(-2.5*np.log10(flux_ch1/flux_ch2))

    if abs(z-2.3) < 0.2:
        ch3_ch4_protoz.append(-2.5*np.log10(flux_ch3/flux_ch4))
        ch1_ch2_protoz.append(-2.5*np.log10(flux_ch1/flux_ch2))
    if abs(z-3) < 0.2:
        ch3_ch4_z3.append(-2.5*np.log10(flux_ch3/flux_ch4))
        ch1_ch2_z3.append(-2.5*np.log10(flux_ch1/flux_ch2))



ch3_ch4 = np.array(ch3_ch4)
ch1_ch2 = np.array(ch1_ch2)
ch3_ch4_protoz = np.array(ch3_ch4_protoz)
ch1_ch2_protoz = np.array(ch1_ch2_protoz)
ch3_ch4_z3 = np.array(ch3_ch4_z3)
ch1_ch2_z3 = np.array(ch1_ch2_z3)

#fill_between([15,24],np.max(ch2_ch4_z15),np.min(ch2_ch4_z15),facecolor='blue',alpha=0.2)
#fill_between([15,24],np.max(ch2_ch4_protoz),np.min(ch2_ch4_protoz),facecolor='green',alpha=0.2)
#fill_between([15,24],np.max(ch2_ch4_z3),np.min(ch2_ch4_z3),facecolor='red',alpha=0.2)

#pl.text(15.5,np.mean(ch2_ch4_z15),r'$z=1.6\pm0.2$',size=12)
#pl.text(15.5,np.mean(ch2_ch4_protoz),r'$z=2.3\pm0.2$',size=12)
#pl.text(15.5,np.mean(ch2_ch4_z3),r'$z=3\pm0.2$',size=12)

#pl.plot(ch1_ch2,ch3_ch4,'k-')
pl.plot(movingaverage(ch1_ch2,20),ch3_ch4,'k-')

#pl.plot(ch1_ch2_protoz,ch3_ch4_protoz,'g-')
#pl.plot(ch1_ch2_z3,ch3_ch4_z3,'r-')

pl.axhspan(min(ch3_ch4_protoz),max(ch3_ch4_protoz),facecolor='g',alpha=0.2)
pl.axhspan(min(ch3_ch4_z3),max(ch3_ch4_z3),facecolor='r',alpha=0.2)

pl.text(-0.7,np.mean(ch3_ch4_protoz),r'$z=2.3\pm0.2$',size=12)
pl.text(-0.7,np.mean(ch3_ch4_z3),r'$z=3\pm0.2$',size=12)



pl.xlim(-0.8,0.6)
pl.ylim(-0.5,1)
pl.savefig('../Figures/Colour/h1700/cc_ch1ch2_ch3ch4.pdf',bbox_inches='tight')
#pl.show()
pl.close()
'''
...
'''
#ch4-ch2 vs ch3-ch1
print 'ch4-ch2 vs ch3-ch1'
fig = pl.figure()
pl.subplots_adjust(hspace=0,wspace=0)
pl.rc('font',size=15)
pl.rc('mathtext', default='regular')
pl.ylabel(r'log(S$_{8.0}$/S$_{4.5}$)')#[5.8] - [3.6]')
pl.xlabel(r'log(S$_{5.8}$/S$_{3.6}$)')#[8.0] - [4.5]')

for i in range(len(K)):
    if ch1[i] > 0 and ch2[i] > 0 and ch3[i] > 0 and ch4[i]>0:

        y = np.log10(10**(-ch4[i]/2.5) / 10**(-ch2[i]/2.5))
        x = np.log10(10**(-ch3[i]/2.5) / 10**(-ch1[i]/2.5))

        pl.scatter(x,y,s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(x,y,ID[i][6:],size=12)
#        pl.scatter(ch3[i]-ch1[i],ch4[i]-ch2[i],s=30,marker='o',facecolors='blue',edgecolors='black')
#        pl.text(ch3[i]-ch1[i],ch4[i]-ch2[i],ID[i][6:],size=12)
    #        if ch3[i]-ch4[i]>0.25:
    #            print ID[i]
    else:
        pass
...
for z in zspace:
    ...
    why = np.log10(Sch4/Sch2)
    ex = np.log10(Sch3/Sch1)
...
pl.plot(movingaverage(x,1),y,'k-')
pl.plot(movingaverage(x_z15,1),y_z15,'b-')
pl.plot(movingaverage(x_protoz,1),y_protoz,'g-')
pl.plot(movingaverage(x_z3,1),y_z3,'r-')

#pl.xlim(-0.8,0.6)
#pl.ylim(-0.5,1)
pl.savefig('../Figures/Colour/h1700/cc_ch4ch2_ch3ch1.pdf',bbox_inches='tight')
pl.show()
pl.close()
'''
...
for i in range(len(K)):
    if ch4[i] > 0 and ch2[i] > 0 and K[i]>0:
        pl.scatter(K[i]-ch2[i],ch2[i]-ch4[i],s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(K[i]-ch2[i],ch2[i]-ch4[i],ID[i][6:],size=12)
#        if ch4[i]-ch2[i]>0.25:
#            print ID[i]
    else:
        pass
...
for z in zspace:
    ...
    Sch4 = S[np.where(wavelength*(1+z)>=8.0)[0][0]]
    Sch2 = S[np.where(wavelength*(1+z)>=4.5)[0][0]]
    SK = S[np.where(wavelength*(1+z)>=2.2)[0][0]]

    mch4 = -2.5*np.log10(Sch4/3631)
    mch2 = -2.5*np.log10(Sch2/3631)
    mK = -2.5*np.log10(SK/3631)
...
y = np.array(y)
x = np.array(x)
y_protoz = np.array(y_protoz)
x_protoz = np.array(x_protoz)
y_z3 = np.array(y_z3)
x_z3 = np.array(x_z3)
y_z15 = np.array(y_z15)
x_z15 = np.array(x_z15)

pl.plot(movingaverage(x,10),y,'k-')
pl.plot(movingaverage(x_z15,2)[1:],y_z15[1:],'b-')
pl.plot(movingaverage(x_protoz,2)[1:],y_protoz[1:],'g-')
pl.plot(movingaverage(x_z3,2)[1:],y_z3[1:],'r-')

pl.savefig('../Figures/Colour/h1700/cc_Kch2_ch2ch4.pdf',bbox_inches='tight')
pl.show()
pl.close()


#ch2 vs ch2-ch4
print 'ch2 vs ch2-ch4'
fig = pl.figure()
pl.subplots_adjust(hspace=0,wspace=0)
pl.rc('font',size=15)
pl.rc('mathtext', default='regular')
...
for i in range(len(K)):
    if ch4[i] > 0 and ch2[i] > 0:
        pl.scatter(ch2[i],ch2[i]-ch4[i],s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(ch2[i],ch2[i]-ch4[i],ID[i][6:],size=12)
#        if ch4[i]-ch2[i]>-0.25:
#            print ID[i]
    else:
        pass
...
for z in zspace:
    ch2_ch4.append(-2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-4.5)<0.002)[0][0]] - -2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-8)<0.005)[0][0]])
    ch2_mag.append(-2.5*np.log10((S[np.where(abs(wavelength*(1+z)-4.5) < 0.01)[0][0]])/3631))

    if abs(z-2.3) < 0.2:
        ch2_ch4_protoz.append(-2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-4.5)<0.002)[0][0]] - -2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-8)<0.005)[0][0]])
        ch2_mag_protoz.append(-2.5*np.log10((S[np.where(abs(wavelength*(1+z)-4.5) < 0.01)[0][0]])/3631))

    if abs(z-3) < 0.2:
        ch2_ch4_z3.append(-2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-4.5)<0.002)[0][0]] - -2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-8)<0.005)[0][0]])
        ch2_mag_z3.append(-2.5*np.log10((S[np.where(abs(wavelength*(1+z)-4.5) < 0.01)[0][0]])/3631))

    if abs(z-1.6) < 0.2:
        ch2_ch4_z15.append(-2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-4.5)<0.002)[0][0]] - -2.5*np.log10(S/3631.0)[np.where(abs(wavelength*(1+z)-8)<0.005)[0][0]])
        ch2_mag_z15.append(-2.5*np.log10((S[np.where(abs(wavelength*(1+z)-4.5) < 0.01)[0][0]])/3631))


ch2_ch4 = np.array(ch2_ch4)
ch2_mag = 1.75*np.array(ch2_mag)
ch2_ch4_protoz = np.array(ch2_ch4_protoz)
ch2_mag_protoz = 1.75*np.array(ch2_mag_protoz)

ch2_ch4_z15 = np.array(ch2_ch4_z15)
ch2_mag_z15 = 1.75*np.array(ch2_mag_z15)
ch2_ch4_z3 = np.array(ch2_ch4_z3)
ch2_mag_z3 = 1.75*np.array(ch2_mag_z3)
...
#pl.plot(ch2_mag,ch2_ch4,'k-')
pl.plot(movingaverage(ch2_mag,20)[11:-9],ch2_ch4[11:-9],'k-')
#pl.plot(ch2_mag_protoz,ch2_ch4_protoz,'g-')
#pl.plot(ch2_mag_z15,ch2_ch4_z15,'b-')
#pl.plot(ch2_mag_z3,ch2_ch4_z3,'r-')
pl.plot(17,1,'w.',label='HS1700')

pl.legend(fontsize=15,loc=2,ncol=1,frameon=False,numpoints=1,scatterpoints=1)

pl.ylim(-1,1.5)
pl.xlim(15,24)
pl.savefig('../Figures/Colour/h1700/cm_ch2_ch2ch4.pdf',bbox_inches='tight')
pl.show()
pl.close()
'''
'''
...
for i in range(len(K)):
    if K[i] > 0 and ch2[i] > 0:
        pl.scatter(K[i],K[i]-ch2[i],s=30,marker='o',facecolors='blue',edgecolors='black')
        pl.text(K[i],K[i]-ch2[i],ID[i][6:],size=12)
    else:
        pass
pl.savefig('../Figures/Colour/h1700/cm_K_Kch2.pdf',bbox_inches='tight')
#pl.show()
pl.close()
'''
